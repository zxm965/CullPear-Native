...
Release 构建启动时会读取 `zxm965/CullPear-Native` 的公开 Release Atom 源，自动检查最多每 6 小时一次，设置页仍可手动强制检查；Debug 开发构建禁用自动与手动更新检查。发现更高版本后会弹窗展示版本号和发行说明；用户确认后使用 Android DownloadManager 后台下载 APK，下载完成后再次弹窗提示安装。该方案不调用有匿名频率限制的 GitHub REST API。
